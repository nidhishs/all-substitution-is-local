...
In the human-AI specialization (paper Appendix B), `kI` is the model-signal channel and `kJ`
is the human-signal channel; the identity decomposes the model-then-human VoI into
per-outer-posterior local terms.
...
This is the quantitative local characterization of `sign(ΔVoI)` (paper §4).
...
This is the local geometric characterization of `sign(ΔVoI)` per the paper §4. -/
